REBAR = /usr/local/lib/erlang/otp-17.1/lib/erlang/bin/escript rebar
INSTALL = /usr/bin/install -c
SED = /bin/sed
ERL = /usr/local/lib/erlang/otp-17.1/bin/erl
...
install: all copy-files
	...
	$(SED) -e "s*{{rootdir}}*/usr/local*g" \
		...
		-e "s*{{erl}}*/usr/local/lib/erlang/otp-17.1/bin/erl*g" \
		-e "s*{{epmd}}*/usr/local/lib/erlang/otp-17.1/bin/epmd*g" ejabberdctl.template \
		> ejabberdctl.example
